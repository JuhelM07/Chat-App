...
final class ChatViewModel: NSObject, ObservableObject {
    enum AppState: String {
        ...
        case hostingAwaitingPeers = "Hosting awaiting peers"
        case hostingWithPeers = "Hosting with peers"
    ...
    static var safeAreaInsetsBottom: CGFloat { UIApplication.shared.windows.first(where: { $0.isKeyWindow })?.safeAreaInsets.bottom ?? 0 }
    
    
    @Published private(set) var appState = AppState.inactive
    @Published var newMessageText = ""
    
    @Published private(set) var messages = [Message(username: User.local.name, value: "Hello World!", timeStamp: "", languageCode: "en", translationLanguageCode: "", translatedValue: "")]
...
//    }
    ...
    override init() {
        ...
        NotificationCenter.default.publisher(for: UIResponder.keyboardWillChangeFrameNotification)
            ...
            .map {$0 * -1 + Self.safeAreaInsetsBottom }
    ...
    func send() {
        ...
        let message = Message(username: User.local.name, value: newMessageText, timeStamp: timeStamp, languageCode: "", translationLanguageCode: "", translatedValue: "")
    ...
    func fetchTranslation(for message: Message, to translationLanguageCode: String) -> AnyPublisher<Message, Never> {
        ...
        return translationService.publisher(for: message, to: translationLanguageCode)
            ...
            .map { translatedValue in
                Message(username: message.username, value: message.value, timeStamp: message.timeStamp, languageCode: message.languageCode, translationLanguageCode: translationLanguageCode, translatedValue: translatedValue == message.value ? "" : translatedValue)
            }
